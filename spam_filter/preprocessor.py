...
class Cleaner(object):
    ...
    def punctuation_remover(data):
        ...
        try:
            ...
            for i in [2534, 2535, 2536, 2537, 2538, 2539, 2540, 2541, 2542, 2543, 2404, 55357, 56842, 55356, 57198, 57252]:
    ...
    @staticmethod
    def remove_num(question):
        return re.sub(ur'[^\D]', '', question)
...
class Decoder(object):
    ...
    :param end: the range will stop in this id (including this)
    """
    def decode_in_range(self, connection_cursor, start, end, table_name='questions', fields='body'):
        while start <= end:
    ...
    def decode(self, question):
        try:
            if question['source'] == 'app':
                return self.html_entity_decode(question['body'])
            else:
                if not self.if_in_english(question['body']):
                    return self.utf8_decode(question['body'])
                else:
